...
object RezeptResolver {
    ...
    suspend fun resolve(
        ...
        for (pos in positionen) {
            ...
            if (pos.subRezeptId != null) {
                ...
                // Sub-Rezept: mengeG ist bereits die skalierte Gesamtmenge des Sub-Rezepts.
                // Das Sub-Rezept intern mit Faktor 1.0 auflösen und dann alle Zutaten
                // anteilig über den Mengenfaktor (mengeG / sub.gesamtGrammRoh) skalieren.
                val subRaw = resolve(
                ...
                // Skalierung anwenden: mengeG / subRaw.gesamtGrammRoh
                val subSkalierung = if (subRaw.gesamtGrammRoh > 0)
                    mengeG / subRaw.gesamtGrammRoh else 1.0
                val skalierteZutaten = subRaw.zutaten.map { z ->
                    z.copy(mengeG = z.mengeG * subSkalierung)
                }
                resolved.addAll(skalierteZutaten)
    ...
    interface Loader {
        ...
        suspend fun loadRezept(rezeptId: Long): com.allernutri.app.data.local.entity.RezeptEntity?
    }
}
