 ...
 * und vergleicht mit dem Bedarf bei [kcalME] und [lebensphase].
 *
 * Lebensphase skaliert den Bedarfswert je Nährstoff nach NRC 2006.
 * ADULT = keine Skalierung (Faktor 1.0 für alle Nährstoffe).
 */
class RezeptAnalyseUseCase {
    ...
    fun analyse(
        ...
        kcalME: Double,
        lebensphase: NrcLebensphasen.Lebensphase = NrcLebensphasen.Lebensphase.ADULT
        ...
        return NaehrstoffKatalog.alle.map { naehrstoff ->
            ...
            // Bedarf wird durch Lebensphase skaliert
            val bedarfPro1000kcalSkaliert = NrcLebensphasen.bedarfPro1000kcal(naehrstoff, lebensphase)
            val bedarfSkaliert = if (kcalME > 0)
                bedarfPro1000kcalSkaliert * kcalME / 1000.0 else 0.0
            val prozent = if (bedarfSkaliert > 0) ist / bedarfSkaliert * 100.0 else 0.0
            NaehrstoffErgebnis(
                naehrstoff  = naehrstoff,
                istWert     = ist,
                bedarfswert = bedarfSkaliert,
                maxWert     = naehrstoff.maxPro1000kcal?.let { it * kcalME / 1000.0 },
                prozent     = prozent
            )
        }
    }
