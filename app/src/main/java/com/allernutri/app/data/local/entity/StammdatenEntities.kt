...
data class ZutatEntity(
    ...
    // Tablette
    val tabletteGewichtG: Double = 0.0,
    // Tropfen
    val tropfenGewichtG: Double = 0.0,
    val tropfenVolumenMl: Double = 0.0,
    // Pulver: Eingabe immer pro 100g → kein Extra-Feld nötig
    val vitaminEForm: String = "natuerlich",
    // Datenquelle (USDA, Open Food Facts, Edamam, manuell)
    val quelle: String = "manuell",
    val quelleId: String = "",              // FDC-ID, Barcode, foodId
    val aktiv: Boolean = true,
    val createdAt: Instant = Instant.now(),
    val deleted: Boolean = false,
    val deletedAt: Instant? = null
...
data class RezeptEntity(
    ...
    val kategorie: String = "",   // z.B. "Fleisch/Rind", "Gemüse/Wurzel", "Supplement"
    val createdAt: Instant = Instant.now(),
    val deleted: Boolean = false,
    val deletedAt: Instant? = null
)
