...
import com.allernutri.app.data.local.dao.TagebuchDao
...
class TagebuchRepository @Inject constructor(private val dao: TagebuchDao) {
    ...
    // ── Export: Snapshot-Listen ───────────────────────────────────────────
    suspend fun allergenList(hundId: Long): List<TagebuchAllergenEntity> =
        dao.getAllergenForHund(hundId).first()

    suspend fun medikamentList(hundId: Long): List<TagebuchMedikamentEntity> =
        dao.getMedikamentForHund(hundId).first()

    suspend fun ausschlussList(hundId: Long): List<TagebuchAusschlussEntity> =
        dao.getAusschlussForHund(hundId).first()

    suspend fun tierarztList(hundId: Long): List<TagebuchTierarztEntity> =
        dao.getTierarztForHund(hundId).first()

    suspend fun futterRange(hundId: Long, von: LocalDate, bis: LocalDate): List<TagebuchFutterEntity> =
        dao.getFutterRange(hundId, von, bis)
}
