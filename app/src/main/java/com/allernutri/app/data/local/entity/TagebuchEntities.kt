package com.allernutri.app.data.local.entity
...
// ─────────────────────────────────────────────
// Symptom-Medien (Fotos / Videos)
// ─────────────────────────────────────────────

/**
 * Foto oder Video das einem Symptom-Eintrag zugeordnet ist.
 * pfad: absoluter Dateipfad im App-eigenen Verzeichnis
 * typ: "foto" | "video"
 * dateigroesseBytes: für UI-Anzeige und Speicherwarnung
 */
@Entity(
    tableName = "symptom_media",
    foreignKeys = [ForeignKey(
        entity        = TagebuchSymptomEntity::class,
        parentColumns = ["id"],
...
data class SymptomMediaEntity(
    @PrimaryKey(autoGenerate = true) val id: Long = 0,
    val symptomId: Long,
    val pfad: String,                       // /data/data/.../files/media/symptom_123_abc.jpg
    val typ: String = "foto",               // "foto" | "video"
    val dateigroesseBytes: Long = 0,
    val createdAt: Instant = Instant.now()
)
