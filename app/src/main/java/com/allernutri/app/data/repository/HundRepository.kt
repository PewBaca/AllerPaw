...
import com.allernutri.app.data.local.dao.HundDao
...
class HundRepository @Inject constructor(private val dao: HundDao) {
    ...
    /** Aktualisiert das aktuelle Gewicht im Hund-Profil (für RER-Berechnung) */
    suspend fun updateGewicht(hundId: Long, kg: Double) =
        dao.updateHundGewicht(hundId, kg)

    suspend fun updateKcalBedarfManuell(hundId: Long, kcal: Double?) =
        dao.updateKcalBedarfManuell(hundId, kcal)
}
