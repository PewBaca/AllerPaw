...
import com.allernutri.app.ui.tagebuch.tabs.*

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun TagebuchScreen(
    onNavigateToSettings: () -> Unit,
    onNavigateToKreuzallergen: () -> Unit,
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.tagebuch_title)) },
                actions = {
                    // Kreuzallergie-Analyse Button im Allergen-Tab
                    if (state.aktuellerTab == TagebuchTab.ALLERGEN) {
                        IconButton(onClick = onNavigateToKreuzallergen) {
                            Icon(Icons.Default.BubbleChart, "Kreuzallergie-Analyse")
                        }
                    }
                    IconButton(onClick = onNavigateToSettings) {
                        Icon(Icons.Default.Settings, stringResource(R.string.cd_einstellungen))
                    }
                }
            )
        },
